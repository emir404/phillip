...
import { SetBreadcrumb } from "@/components/breadcrumb-title";
...
export default async function LeadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <SetBreadcrumb label={dl.lead.business} />
      <AutoRefresh />
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2.5">
              ...
              {orderPaid && dl.order ? (
                <span className="inline-flex h-5 items-center rounded-full bg-emerald-50 px-2 text-xs font-medium text-emerald-700 tabular-nums dark:bg-emerald-400/15 dark:text-emerald-300">
                  paid · {money(dl.order.amount, dl.order.currency)}
                </span>
              ) : null}
            </div>
      ...
      <div className="grid items-start gap-4 md:grid-cols-2 xl:grid-cols-3">
        {/* The conversation is the hero: it follows the scroll on wide screens. */}
                  ...
                  <ol className="mt-2 flex flex-col gap-2">
                    {brief.recommendedActions.map((a, i) => (
                      <li key={a} className="flex items-start gap-2.5 text-sm">
                        <span
                          aria-hidden
                          className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-md bg-brand/10 text-[11px] font-semibold text-brand tabular-nums dark:bg-brand/18 dark:text-brand-start"
                        >
                          {i + 1}
                        </span>
                ...
                <div>
                  ...
                  <ul className="mt-2 flex flex-col gap-1.5">
                    {brief.requestedChanges.map((c) => (
                      <li
                        key={c}
                        className="rounded-lg border-l-2 border-brand/40 bg-muted/50 px-2.5 py-1.5 text-sm leading-snug"
                      >
                        “{c}”
                      </li>
                    ))}
                ...
                <div>
                  ...
                  <ul className="mt-2 flex flex-col gap-1.5">
                    {brief.objections.map((o) => (
                      <li
                        key={o}
                        className="rounded-lg border-l-2 border-rose-400/60 bg-muted/50 px-2.5 py-1.5 text-sm leading-snug"
                      >
                        {o}
                      </li>
                    ))}
