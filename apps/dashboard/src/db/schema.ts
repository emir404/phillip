...
// A lead's attached photo/file (e.g. "use this as my logo"), decoded from the
// `data:` URL the widget sent and re-hosted so the executor's prompt can
// reference a real URL instead of carrying the base64 payload around. Stored
// as base64 text rather than a native blob column — libsql-over-HTTP (Turso)
// handles a text column uniformly across local sqlite and prod without
// driver-specific blob binding, and attachments are already capped small
// client-side (4MB raw, images downscaled to ~640px) before they get here.
