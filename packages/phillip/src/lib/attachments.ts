...
// (or, in live mode, pay to host it) for what's usually going in a nav bar at
// ~120px wide. Non-image files (a PDF menu, etc.) are just read as-is under a
// hard size cap, since there's no cheap way to "downscale" a document.
...
/** Reads a File into an Attachment, downscaling images so payloads stay small. */
export async function readAttachment(file: File): Promise<Attachment> {
  if (file.size > MAX_FILE_BYTES) {
    throw new Error(`${file.name} is too large (max ${MAX_FILE_BYTES / (1024 * 1024)}MB)`);
  }
  const raw = await readAsDataUrl(file);
  const dataUrl = file.type.startsWith("image/") ? await downscaleImage(raw, file.type) : raw;
  return { name: file.name, mediaType: file.type || "application/octet-stream", dataUrl };
}
